...
#include "fake_dlfcn.hpp"

#define TAG_NAME	"jni_test"
...
#define fatal(fmt,args...) do { log_err(fmt,##args); goto err_exit; } while(0)
	...
	for(k = 0; k < elf->e_shnum; k++, shoff += elf->e_shentsize)  {
		...
		case SHT_DYNSYM:
		    ...
		    memcpy(ctx->dynsym, ((uint8_t *) elf) + sh->sh_offset, sh->sh_size);
		...
		case SHT_STRTAB:
		    ...
		    memcpy(ctx->dynstr, ((uint8_t *) elf) + sh->sh_offset, sh->sh_size);
		    break;
...
void *fake_dlsym(void *handle, const char *name)
{
	...
	if(strcmp(strings + sym->st_name, name) == 0) {
	    ...
	    void *ret = ((uint8_t*)ctx->load_addr) + sym->st_value - ctx->bias;
